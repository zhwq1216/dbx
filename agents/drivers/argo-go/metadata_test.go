...
// The argo agent always reports ArgoDB identity; Kyuubi/Impala/Hive remain on hive-go.
func TestConnectionInfoReportsArgoIdentity(t *testing.T) {
...
// Unlike hive-go (which gates routine listing on the connection's database_type so
// vanilla Apache Hive never fires the catalog-view queries), the argo agent serves
// 星环Argo (Transwarp ArgoDB) exclusively and must always query the routine views.
func TestListObjectsQueriesRoutineViewsUnconditionally(t *testing.T) {
	behavior := &scriptedBehavior{
		query: func(ctx context.Context, query string) (driver.Rows, error) {
			switch {
			case strings.Contains(strings.ToUpper(query), "PROCEDURES_V"):
				return newScriptedRows(ctx, []string{"procedure_name"}, []string{"STRING"}, [][]driver.Value{{"sp_etl_log"}}), nil
			case strings.Contains(strings.ToUpper(query), "FUNCTIONS_V"):
				return newScriptedRows(ctx, []string{"function_name"}, []string{"STRING"}, nil), nil
			default:
				return newScriptedRows(ctx, []string{"tab_name"}, []string{"STRING"}, [][]driver.Value{{"events"}}), nil
			}
		},
	...
	for _, v := range values {
		if v.ObjectType == "PROCEDURE" && v.Name == "sp_etl_log" {
			found = true
		}
	}
	if !found {
		t.Fatalf("argo agent must surface routines regardless of database_type, got: %+v", values)
	}
}
