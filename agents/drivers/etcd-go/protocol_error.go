...
// isUnavailableError treats gRPC UNAVAILABLE (etcd server unreachable) as a
// connection failure, matching the Java agent's transport classification.
func isUnavailableError(err error) bool {
	...
	for _, marker := range []string{
		...
		"connection lost", "unexpected eof", "no route to host",
	} {
...
func rpcErrorStage(method string) string {
	...
	case "close_session", "disconnect", "shutdown":
		return "close"
	case "etcd_watch_poll", "etcd_lease_get":
		return "fetch"
	case "handshake", "":
		return "request"
	default:
