...
```html
<a href="dbx://open">使用 DBX 打开</a>
```

使用 `dbx://connection/new` 可以打开新建连接窗口并预填连接信息。
...
<Callout type="info">测试系统级 `dbx://` 唤起前，请先安装并启动一次 DBX 桌面端，让系统注册 URL 协议。macOS 可使用 `open 'dbx://open'` 或 `open 'dbx://connection/new?...'` 测试。</Callout>
