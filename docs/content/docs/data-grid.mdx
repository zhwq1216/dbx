...
- **Update by `WHERE`** in MySQL table-data views to set one writable column for every database row matching a condition you enter, including rows outside the loaded page
...
Row-level relational mutations remain local until saved. DBX then shows the generated `INSERT`, `UPDATE`, and `DELETE` statements.

A MySQL condition-wide bulk update is different because not all matching rows are necessarily loaded into the grid. In the bulk-edit dialog, use the current table filter or build matching conditions by choosing columns, operators, and values; combine multiple rules with `AND` or `OR`. A custom `WHERE` expression remains available as an advanced option. You can preview the estimated affected-row count before continuing; it is a snapshot and the actual count can differ when the update executes. DBX shows the exact `UPDATE ... WHERE ...` statement in a danger confirmation and executes it immediately after confirmation. The database evaluates the condition at execution time, and DBX cannot generate reliable rollback SQL for rows that were not loaded.
