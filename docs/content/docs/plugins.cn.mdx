...
<Callout type="info">本页介绍的是 JDBC 插件和厂商 Driver JAR。若你要开发 SSH、SFTP、文件管理器或其他 DBX 扩展，并提交官方插件商店，请阅读[开发和提交 DBX 插件](/cn/docs/plugin-development)。</Callout>
...
<Callout type="info">Oracle 驱动请同时导入与 `ojdbc` 同版本的 `com.oracle.database.nls:orai18n`。基础 ojdbc jar 不支持 `ZHS16GBK` 等多字节字符集，缺少它时表/字段注释等元数据无法加载。通过 Maven 安装 `com.oracle.database.jdbc:ojdbc*` 时会自动附带同版本的 `orai18n`。使用 DBX 内置 Oracle 连接（默认驱动）则无需额外 JAR，已支持这些字符集。</Callout>
