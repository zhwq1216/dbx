...
<Callout type="info">This page covers the JDBC plugin and vendor driver JARs. To build SSH, SFTP, file-manager, or other DBX extensions and submit them to the official Marketplace, read [Develop and Submit DBX Plugins](/en/docs/plugin-development).</Callout>
...
<Callout type="info">For Oracle, also import `com.oracle.database.nls:orai18n` with the same version as `ojdbc`. The base ojdbc jar cannot convert multibyte charsets such as `ZHS16GBK`, so table and column comments fail to load without it. Maven installs of `com.oracle.database.jdbc:ojdbc*` fetch the matching `orai18n` jar automatically. DBX's built-in Oracle connection (default driver) supports these charsets without any extra JAR.</Callout>
