...
| Connection read-only | Blocks recognizable writes in the DBX core query path and disables multiple mutation entry points | AI, CLI, and MCP cannot widen it. The workbench can unlock writes for 1 or 5 minutes after explicit confirmation; the saved setting stays on |
...
### Temporary write unlock

In the desktop and web workbench, a read-only connection does not silently drop writes. A mutating action opens a confirmation dialog. Choose **1 minute** or **5 minutes** (default 1 minute). After confirm:

- Writes on that connection succeed without another prompt until the timer ends
- Remaining time is shown on the read-only control; **Lock writes now** ends the window early
- The saved **Read-only connection** setting stays on
- Unlocking one connection does not unlock others
- When the window expires, the next write prompts again

MCP and CLI have no confirmation UI and keep using the saved read-only flag, so they cannot use this window.
...
| --- | --- | --- |
| SQL editor | Rejects recognizable writes unless the user confirms a 1/5 minute unlock window | Shows the target connection, database, source, and SQL for per-operation confirmation |
| Data grid | Disables or rejects saves, deletes, and other mutations until the same unlock window is confirmed | Row saves and deletes enter the shared production confirmation flow |
| Table structure and object actions | Hides or disables recognizable mutation entry points until the unlock window is confirmed | DDL, source saves, and database administration enter the shared confirmation flow |
...
| Data Transfer | Cannot target a read-only connection; **Drop Target Before Create** is blocked on production databases | Dangerous table-replacement operations require explicit confirmation and show DROP SQL preview |
...
| MCP | Connection read-only is an upper bound for every mode; the workbench unlock window does not apply | Production protection remains an upper bound for MCP policy |
