...
where
    D: Deserializer<'de>,
{
    Ok(Vec::<String>::deserialize(deserializer)?.into_iter().map(OsString::from).collect::<Vec<_>>())
}
...
pub(crate) fn validate_endpoints(endpoints: &[Url], dangerous_insecure_transport_protocol: bool) -> crate::Result<()> {
