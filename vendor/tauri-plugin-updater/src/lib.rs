...
impl<R: Runtime, T: Manager<R>> UpdaterExt<R> for T {
    fn updater_builder(&self) -> UpdaterBuilder {
        ...
        let UpdaterState { config, target, version_comparator, headers } = self.state::<UpdaterState>().inner();
    ...
    pub fn build<R: Runtime>(self) -> TauriPlugin<R, Config> {
        ...
        PluginBuilder::<R, Config>::new("updater")
            .setup(move |app, api| {
                ...
                app.manage(UpdaterState { target, config, version_comparator, headers });
                Ok(())
            })
